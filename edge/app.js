...
const getDetails = (search) => {
  try {
    ...
    if (!/^([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/.test(bg)) throw new Error();
    if (!/^([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/.test(fg)) throw new Error();
    // Check width/height - Should be a decimal number > 0, is optional
    if (width && !/^\d+(\.\d+)?$/.test(width)) throw new Error();
    if (height && !/^\d+(\.\d+)?$/.test(height)) throw new Error();
    // Check text - Should be letters, numbers and spacebars, also special spanish letters
    const raw_text = decodeURIComponent(content);
    if (!/^[A-Za-z0-9ÁÉÍÓÚáéíóúÑñÜü ]+$/.test(raw_text)) throw new Error();
